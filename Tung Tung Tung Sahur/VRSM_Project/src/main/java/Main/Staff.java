...
private void addAnalyticsReportButton() {
    ...
    private void initSystemHistoryPanel() {
    // Clear existing components
    systemhistroy.removeAll();
    
    // Set modern layout with proper padding
    systemhistroy.setLayout(new BorderLayout(0, 10));
    systemhistroy.setBorder(BorderFactory.createEmptyBorder(15, 15, 15, 15));
    systemhistroy.setBackground(new Color(240, 248, 255));

    // Create a stylish header panel with filters
    ...
    filtersPanel.setBackground(new Color(240, 248, 255));
    
    // Date range filter
    JLabel fromLabel = new JLabel("From:");
    fromLabel.setFont(new java.awt.Font("Segoe UI", java.awt.Font.PLAIN, 12));
    filtersPanel.add(fromLabel);
    
    JDateChooser fromDateChooser = new JDateChooser();
    fromDateChooser.setDateFormatString("yyyy-MM-dd");
    fromDateChooser.setPreferredSize(new Dimension(110, 28));
    filtersPanel.add(fromDateChooser);
    ...
    toDateChooser.setDate(new Date()); // Set to current date
    toDateChooser.setPreferredSize(new Dimension(110, 28));
    filtersPanel.add(toDateChooser);
    
    // Event type filter
    JLabel typeLabel = new JLabel("Event Type:");
    typeLabel.setFont(new java.awt.Font("Segoe UI", java.awt.Font.PLAIN, 12));
    filtersPanel.add(typeLabel);
    
    String[] eventTypes = {
        "All Events", "Rentals", "Returns", "Payments", "Status Changes", 
        "Machine Updates", "Reservations"
    };
    JComboBox<String> typeFilter = new JComboBox<>(eventTypes);
    typeFilter.setPreferredSize(new Dimension(130, 28));
    ...
    // Add title and filters to header
    headerPanel.add(titlePanel, BorderLayout.WEST);
    headerPanel.add(filtersPanel, BorderLayout.EAST);
    
    // Create the table model for history
    DefaultTableModel historyModel = new DefaultTableModel(
        new Object[][] {},
        new String[] {"Date & Time", "Event Type", "Description", "Machine", "Customer", "User", "Status"}
    ) {
        @Override
    ...
    historyTable.getTableHeader().setFont(new java.awt.Font("Segoe UI", java.awt.Font.BOLD, 12));
    ...
    // Set column widths
    historyTable.getColumnModel().getColumn(0).setPreferredWidth(140); // Date & Time
    historyTable.getColumnModel().getColumn(1).setPreferredWidth(100); // Event Type
    historyTable.getColumnModel().getColumn(2).setPreferredWidth(250); // Description
    historyTable.getColumnModel().getColumn(3).setPreferredWidth(100); // Machine
    historyTable.getColumnModel().getColumn(4).setPreferredWidth(120); // Customer
    historyTable.getColumnModel().getColumn(5).setPreferredWidth(80);  // User
    historyTable.getColumnModel().getColumn(6).setPreferredWidth(100); // Status
    
    // Create scroll pane for the table
    JScrollPane historyScrollPane = new JScrollPane(historyTable);
    historyScrollPane.setBorder(BorderFactory.createLineBorder(new Color(220, 220, 220)));
    ...
    // Status count panel on the left
    JPanel statusPanel = new JPanel(new FlowLayout(FlowLayout.LEFT, 10, 0));
    statusPanel.setBackground(new Color(240, 248, 255));
    
    JLabel statusLabel = new JLabel("0 records found");
    statusLabel.setFont(new java.awt.Font("Segoe UI", java.awt.Font.ITALIC, 12));
    ...
    viewDetailsButton.setBackground(new Color(33, 150, 243));
    ...
    systemhistroy.add(headerPanel, BorderLayout.NORTH);
    systemhistroy.add(historyScrollPane, BorderLayout.CENTER);
    systemhistroy.add(bottomPanel, BorderLayout.SOUTH);
    
    // Add event listeners
    refreshButton.addActionListener(e -> loadSystemHistory(historyModel, null, null, "All Events", ""));
    
    applyFilterButton.addActionListener(e -> {
        Date fromDate = fromDateChooser.getDate();
        Date toDate = toDateChooser.getDate();
        String eventType = (String) typeFilter.getSelectedItem();
        String searchQuery = searchField.getText().trim();
        
        loadSystemHistory(historyModel, fromDate, toDate, eventType, searchQuery);
    });
    
    viewDetailsButton.addActionListener(e -> {
        int selectedRow = historyTable.getSelectedRow();
        if (selectedRow >= 0) {
            selectedRow = historyTable.convertRowIndexToModel(selectedRow); // Convert to model index if sorted
            showEventDetails((String)historyModel.getValueAt(selectedRow, 1), 
                            (String)historyModel.getValueAt(selectedRow, 2));
        } else {
            JOptionPane.showMessageDialog(systemhistroy, 
    ...
    exportButton.addActionListener(e -> exportHistoryToPDF(historyTable));
    
    // Update row count when the model changes
    historyModel.addTableModelListener(e -> {
        int rowCount = historyModel.getRowCount();
        statusLabel.setText(rowCount + (rowCount == 1 ? " record found" : " records found"));
    });
    ...
    // Load initial data
    loadSystemHistory(historyModel, null, null, "All Events", "");
    
    // Color code the rows based on event type
    historyTable.setDefaultRenderer(Object.class, new DefaultTableCellRenderer() {
        ...
        public Component getTableCellRendererComponent(JTable table, Object value,
            ...
            if (!isSelected) {
                String eventType = (String) table.getValueAt(row, 1);
                switch (eventType) {
                    case "Rental":
                        c.setBackground(new Color(237, 247, 237)); // Light green
                        break;
                    case "Return":
                        c.setBackground(new Color(232, 245, 233)); // Slightly darker green
                        break;
                    case "Payment":
                    ...
                    case "Reservation":
                        ...
                        break;
                    case "Machine Update":
                        c.setBackground(new Color(232, 234, 246)); // Light indigo
                        break;
                    default:
    ...
    systemhistroy.revalidate();
    systemhistroy.repaint();
}

/**
 * Load system history data into the table model
 */
private void loadSystemHistory(DefaultTableModel model, Date fromDate, Date toDate, String eventType, String searchQuery) {
    ...
    // Convert Java dates to SQL dates if not null
    ...
    try {
        // We'll collect data from multiple tables to provide a comprehensive history
        
        // 1. Get rental logs
        if ("All Events".equals(eventType) || "Rentals".equals(eventType) || "Returns".equals(eventType)) {
            loadRentalEvents(model, connection, sqlFromDate, sqlToDate, searchQuery);
        }
        
        // 2. Get payment history
        if ("All Events".equals(eventType) || "Payments".equals(eventType)) {
            loadPaymentEvents(model, connection, sqlFromDate, sqlToDate, searchQuery);
        }
        
        // 3. Get machine status changes
        if ("All Events".equals(eventType) || "Status Changes".equals(eventType)) {
            loadStatusChangeEvents(model, connection, sqlFromDate, sqlToDate, searchQuery);
        }
        
        // 4. Get machine updates (add/edit/delete)
        if ("All Events".equals(eventType) || "Machine Updates".equals(eventType)) {
            loadMachineUpdateEvents(model, connection, sqlFromDate, sqlToDate, searchQuery);
        }
        
        // 5. Get reservation events
        if ("All Events".equals(eventType) || "Reservations".equals(eventType)) {
            loadReservationEvents(model, connection, sqlFromDate, sqlToDate, searchQuery);
        }
        
    } catch (SQLException e) {
        e.printStackTrace();
        JOptionPane.showMessageDialog(null, "Error loading history data: " + e.getMessage(), 
                                     "Database Error", JOptionPane.ERROR_MESSAGE);
    }
}

/**
 * Load rental events from rental_logs table
 */
private void loadRentalEvents(DefaultTableModel model, Connection connection, 
                             java.sql.Date fromDate, java.sql.Date toDate, 
                             String searchQuery) throws SQLException {
    StringBuilder queryBuilder = new StringBuilder(
        "SELECT date_created, 'Rental' as event_type, " +
        "CONCAT('Machine \"', machine_name, '\" rented by ', customer_name) as description, " +
        "machine_name, customer_name, 'System' as user_name, Status " +
        "FROM rental_logs WHERE 1=1"
    );
    
    // Add date filters if provided
    if (fromDate != null) {
        queryBuilder.append(" AND date_created >= ?");
    }
    if (toDate != null) {
        queryBuilder.append(" AND date_created <= ?");
    }
    ...
    if (searchQuery != null && !searchQuery.isEmpty()) {
        queryBuilder.append(" AND (machine_name LIKE ? OR customer_name LIKE ? OR status LIKE ?)");
    }
    
    queryBuilder.append(" ORDER BY date_created DESC");
    ...
    if (toDate != null) {
        pstmt.setDate(paramIndex++, toDate);
    }
    ...
    // Also load return events (when status changes to Completed)
    StringBuilder returnQuery = new StringBuilder(
        "SELECT date_created, 'Return' as event_type, " +
        "CONCAT('Machine \"', machine_name, '\" returned by ', customer_name) as description, " +
        "machine_name, customer_name, 'System' as user_name, 'Completed' as Status " +
        "FROM rental_logs WHERE Status = 'Completed'"
    );
    
    // Add date filters if provided
    if (fromDate != null) {
        returnQuery.append(" AND date_created >= ?");
    }
    if (toDate != null) {
        returnQuery.append(" AND date_created <= ?");
    }
    ...
    // Add search filter if provided
    if (searchQuery != null && !searchQuery.isEmpty()) {
        returnQuery.append(" AND (machine_name LIKE ? OR customer_name LIKE ?)");
    }
    
    returnQuery.append(" ORDER BY date_created DESC");
    
    pstmt = connection.prepareStatement(returnQuery.toString());
...
/**
 * Load payment events from payments table
 */
private void loadPaymentEvents(DefaultTableModel model, Connection connection, 
                              java.sql.Date fromDate, java.sql.Date toDate, 
                              String searchQuery) throws SQLException {
    StringBuilder queryBuilder = new StringBuilder(
        "SELECT p.date_created, 'Payment' as event_type, " +
        "CONCAT('Payment of ₱', FORMAT(p.amount_paid, 2), ' received for rental #', p.log_id, ' via ', p.payment_type) as description, " +
        "r.machine_name, r.customer_name, 'System' as user_name, p.payment_type as Status " +
        "FROM payments p JOIN rental_logs r ON p.log_id = r.log_id WHERE 1=1"
    );
    
    // Add date filters if provided
    if (fromDate != null) {
        queryBuilder.append(" AND p.date_created >= ?");
    }
    if (toDate != null) {
        queryBuilder.append(" AND p.date_created <= ?");
    }
    ...
    // Add search filter if provided
    if (searchQuery != null && !searchQuery.isEmpty()) {
        queryBuilder.append(" AND (r.machine_name LIKE ? OR r.customer_name LIKE ? OR p.payment_type LIKE ?)");
    }
    
    queryBuilder.append(" ORDER BY p.date_created DESC");
...
/**
 * Load status change events
 * Note: In a real system, you'd have a status_changes table to track these
 * For this example, we'll use rental_logs and infer status changes
 */
private void loadStatusChangeEvents(DefaultTableModel model, Connection connection, 
                                  java.sql.Date fromDate, java.sql.Date toDate, 
                                  String searchQuery) throws SQLException {
    // For a more complete system, create a status_changes table to track these events properly
    // Here we'll just add a placeholder query for demonstration
    StringBuilder queryBuilder = new StringBuilder(
        "SELECT date_created, 'Status Change' as event_type, " +
        "CONCAT('Machine \"', machine_name, '\" status changed to ', Status) as description, " +
        "machine_name, customer_name, 'System' as user_name, Status " +
        "FROM rental_logs WHERE Status IN ('Pickup', 'Completed', 'Cancelled')"
    );
...
/**
 * Load machine update events
 * Note: In a real system, you'd have an audit_log table to track these
 */
private void loadMachineUpdateEvents(DefaultTableModel model, Connection connection, 
                                   java.sql.Date fromDate, java.sql.Date toDate, 
                                   String searchQuery) throws SQLException {
    // For a more complete system, create an audit_log table
    // Here we'll just use the machine info directly
    StringBuilder queryBuilder = new StringBuilder(
        "SELECT NOW() as date_created, 'Machine Update' as event_type, " +
        "CONCAT('Machine \"', machine_name, '\" status: ', status) as description, " +
        "machine_name, 'N/A' as customer_name, 'System' as user_name, status " +
        "FROM videoke_machines WHERE 1=1"
    );
    
    // Add search filter if provided
    if (searchQuery != null && !searchQuery.isEmpty()) {
        queryBuilder.append(" AND (machine_name LIKE ? OR status LIKE ? OR remarks LIKE ?)");
    }
...
/**
 * Load reservation events from the reservations table
 */
private void loadReservationEvents(DefaultTableModel model, Connection connection, 
                                 java.sql.Date fromDate, java.sql.Date toDate, 
                                 String searchQuery) throws SQLException {
    StringBuilder queryBuilder = new StringBuilder(
        "SELECT date_created, 'Reservation' as event_type, " +
        "CONCAT('Machine \"', machine_name, '\" reserved by ', customer_name, ' from ', " +
        "DATE_FORMAT(reservation_start_date, '%Y-%m-%d'), ' to ', DATE_FORMAT(reservation_end_date, '%Y-%m-%d')) as description, " +
        "machine_name, customer_name, 'System' as user_name, status " +
 ...
 */
private void showEventDetails(String eventType, String description) {
    // Create a dialog to display full details
    ...
    // Create a panel with event details
    JPanel detailsPanel = new JPanel(new BorderLayout(10, 10));
    detailsPanel.setBorder(BorderFactory.createEmptyBorder(15, 15, 15, 15));
    detailsPanel.setBackground(Color.WHITE);
    
    // Create header with event type
    JLabel eventTypeLabel = new JLabel(eventType);
    eventTypeLabel.setFont(new java.awt.Font("Segoe UI", java.awt.Font.BOLD, 18));
    
    // Set icon based on event type
    ImageIcon icon = null;
    Color headerColor = new Color(33, 150, 243); // Default blue
    
    switch (eventType) {
        case "Rental":
            // icon = new ImageIcon(getClass().getResource("/rental_icon.png"));
            headerColor = new Color(76, 175, 80); // Green
            break;
        case "Return":
            // icon = new ImageIcon(getClass().getResource("/return_icon.png"));
            headerColor = new Color(0, 150, 136); // Teal
            break;
        case "Payment":
            // icon = new ImageIcon(getClass().getResource("/payment_icon.png"));
            headerColor = new Color(33, 150, 243); // Blue
            break;
        case "Status Change":
            // icon = new ImageIcon(getClass().getResource("/status_icon.png"));
            headerColor = new Color(255, 152, 0); // Orange
            break;
        case "Machine Update":
            // icon = new ImageIcon(getClass().getResource("/machine_icon.png"));
            headerColor = new Color(156, 39, 176); // Purple
            break;
        case "Reservation":
            // icon = new ImageIcon(getClass().getResource("/reservation_icon.png"));
            headerColor = new Color(233, 30, 99); // Pink
            break;
    }
    
    // If icon is available
    if (icon != null) {
        eventTypeLabel.setIcon(icon);
    }
    ...
    headerPanel.add(eventTypeLabel, BorderLayout.WEST);
    ...
    descriptionHeader.setFont(new java.awt.Font("Segoe UI", java.awt.Font.BOLD, 14));
    descriptionHeader.setAlignmentX(Component.LEFT_ALIGNMENT);
    
    JTextArea descriptionText = new JTextArea(description);
    descriptionText.setFont(new java.awt.Font("Segoe UI", java.awt.Font.PLAIN, 14));
    ...
    // Here you could query the database for more details about this specific event
    // based on eventType and description
    ...
    closeButton.setFont(new java.awt.Font("Segoe UI", java.awt.Font.BOLD, 12));
...
/**
 * Export history table data to PDF
 */
private void exportHistoryToPDF(JTable table) {
    // Create file chooser for saving the report
    JFileChooser fileChooser = new JFileChooser();
    fileChooser.setDialogTitle("Save History Report");
    fileChooser.setSelectedFile(new File("System_History_Report_" + 
                                       new SimpleDateFormat("yyyy-MM-dd").format(new Date()) + ".pdf"));
    fileChooser.setFileFilter(new FileNameExtensionFilter("PDF Files", "pdf"));
    ...
    try {
        Document document = new Document(PageSize.A4.rotate(), 10, 10, 50, 50);
        PdfWriter writer = PdfWriter.getInstance(document, new FileOutputStream(fileToSave));
        
        // Add header with page number
        writer.setPageEvent(new HeaderFooter("System History Report", "Generated on " + 
                                          new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date())));
        document.open();
        
        // Add title
        Font titleFont = new Font(Font.FontFamily.HELVETICA, 18, Font.BOLD, BaseColor.DARK_GRAY);
        Paragraph title = new Paragraph("System Activity History Report", titleFont);
        title.setAlignment(Element.ALIGN_CENTER);
        document.add(title);
        
        // Add date range
        Font dateFont = new Font(Font.FontFamily.HELVETICA, 12, Font.NORMAL, BaseColor.DARK_GRAY);
        Paragraph dateRange = new Paragraph("Report generated on: " + 
                                          new SimpleDateFormat("MMMM dd, yyyy").format(new Date()), dateFont);
        dateRange.setAlignment(Element.ALIGN_CENTER);
        document.add(dateRange);
        
        document.add(Chunk.NEWLINE);
        
        // Create PdfPTable for the history data
        PdfPTable pdfTable = new PdfPTable(table.getColumnCount());
        pdfTable.setWidthPercentage(100);
        
        // Set relative column widths
        float[] columnWidths = {1.5f, 1f, 3f, 1.5f, 1.5f, 1f, 1.2f};
        ...
        for (int i = 0; i < table.getRowCount(); i++) {
            for (int j = 0; j < table.getColumnCount(); j++) {
                ...
                // Set background color based on event type (column 1)
                if (j == 0) { // Don't color the date column
                    cell.setBackgroundColor(BaseColor.WHITE);
                } else {
                    String eventType = (String) table.getValueAt(i, 1);
                    switch (eventType) {
                        case "Rental":
                            cell.setBackgroundColor(new BaseColor(237, 247, 237)); // Light green
                            break;
                        case "Return":
                            cell.setBackgroundColor(new BaseColor(232, 245, 233)); // Slightly darker green
                            break;
                        case "Payment":
                            cell.setBackgroundColor(new BaseColor(227, 242, 253)); // Light blue
                            break;
                        case "Status Change":
                            cell.setBackgroundColor(new BaseColor(255, 243, 224)); // Light orange
                            break;
                        case "Reservation":
                            cell.setBackgroundColor(new BaseColor(243, 229, 245)); // Light purple
                            break;
                        case "Machine Update":
                            cell.setBackgroundColor(new BaseColor(232, 234, 246)); // Light indigo
                            break;
                        default:
                            cell.setBackgroundColor(i % 2 == 0 ? BaseColor.WHITE : new BaseColor(248, 248, 248));
                            break;
                    }
                }
        ...
        // Add legend
        document.add(Chunk.NEWLINE);
        Paragraph legendTitle = new Paragraph("Event Type Legend:", new Font(Font.FontFamily.HELVETICA, 10, Font.BOLD));
        ...
        // Add legend items
        addLegendItem(legendTable, "Rental", new BaseColor(237, 247, 237));
        addLegendItem(legendTable, "Return", new BaseColor(232, 245, 233));
        addLegendItem(legendTable, "Payment", new BaseColor(227, 242, 253));
        addLegendItem(legendTable, "Status Change", new BaseColor(255, 243, 224));
        addLegendItem(legendTable, "Reservation", new BaseColor(243, 229, 245));
        addLegendItem(legendTable, "Machine Update", new BaseColor(232, 234, 246));
        ...
        Paragraph footnote = new Paragraph(
            "This report contains a comprehensive history of all system activities.", 
        ...
        // Show success message with option to open the file
        int openFile = JOptionPane.showConfirmDialog(this,
                "History report created successfully!\nWould you like to open the file?",
                "Report Generated",
                JOptionPane.YES_NO_OPTION,
                JOptionPane.INFORMATION_MESSAGE);
...
/**
 * Add a legend item to the legend table
 */
private void addLegendItem(PdfPTable table, String text, BaseColor color) {
    ...
    // Text cell
    PdfPCell textCell = new PdfPCell(new Phrase(text, new Font(Font.FontFamily.HELVETICA, 8)));
...
/**
 * Custom page event handler for PDF header and footer
 */
private class HeaderFooter extends PdfPageEventHelper {
    ...
    public void onEndPage(PdfWriter writer, Document document) {
        ...
        cb.showTextAligned(PdfContentByte.ALIGN_LEFT, footer, rect.getLeft() + 30, y, 0);
        cb.showTextAligned(PdfContentByte.ALIGN_RIGHT, 
                         "Page " + writer.getPageNumber(), rect.getRight() - 30, y, 0);
        cb.endText();
...
//GEN-LAST:event_machine6bttn1ActionPerformed
    ...
    private void rentActionPerformed(java.awt.event.ActionEvent evt) {
        try {
            ...
            // Extract machine status directly from the info text
            ...
            for (String line : machineInfoText.split("\n")) {
                if (line.startsWith("Machine Name: ")) {
                    ...
                    statusBuilder.append(line.substring("Status: ".length()).trim());
                }
            }
            
            final String machineName = machineNameBuilder.toString();
            final String machineStatus = statusBuilder.toString();
            ...
            // Check if machine is available for direct rental
            if (!"Available".equalsIgnoreCase(machineStatus)) {
                // Machine is not available for direct rental - offer reservation instead
                int reserveChoice = JOptionPane.showConfirmDialog(null, 
                    "This machine is currently " + machineStatus + " and not available for direct rental.\n" +
                    "Would you like to make a reservation instead?", 
                    "Machine Not Available", 
                    JOptionPane.YES_NO_OPTION,
                    JOptionPane.QUESTION_MESSAGE);
                
                if (reserveChoice == JOptionPane.YES_OPTION) {
                    // Redirect to reservation tab
                    jTabbedPane1.setSelectedIndex(5); // Index for Manage Reservation tab
                    
                    // Pre-fill reservation form data if possible - this would depend on how your reservation form works
                    // You might need to create a method to prefill the reservation form
                    // prefillReservationForm(customerName, contactNumber, emailAddress, address, machineName);
                    
                    JOptionPane.showMessageDialog(null, 
                        "Please complete your reservation details in the Reservation tab.",
                        "Make Reservation", 
                        JOptionPane.INFORMATION_MESSAGE);
                }
                return;
            ...
            // Continue with the rental process for available machines
            ...
            // Rest of your existing rental code...
            ...
            if (rs.next()) {
                ...
                // Double-check status from database just to be safe
                String dbStatus = rs.getString("status");
                if (!"Available".equalsIgnoreCase(dbStatus)) {
                    JOptionPane.showMessageDialog(null, "The machine status has changed. It is no longer available.",
                            "Status Changed", JOptionPane.ERROR_MESSAGE);
                    loadMachineTableData(); // Refresh the table
                    return;
                }
            } else {
